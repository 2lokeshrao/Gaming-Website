...
var initialGlobalConfig = {
  ...
  topBannerTemplate: "\u{1F525} Top Verified Gaming Sites available in {{country}} today!",
  ...
  pushNotifications: [
    {
      id: "pn_101",
      title: "\u{1F525} 500% FLASH BONUS ACTIVATED!",
...
var PORT = 3e3;
var JWT_SECRET = process.env.JWT_SECRET || "dev_jwt_secret_fallback_123!";
...
var statePlatforms = [...initialPlatforms];
var stateConfig = { ...initialGlobalConfig };
var stateSubPartners = [];
var stateCustomPages = [];
...
var dbPassword = process.env.DB_PASSWORD || process.env.DB_PASS || "";
var dbName = process.env.DB_NAME || "bonuspromocode_db";
var dbPool = null;
...
async function loadStateFromDb() {
  try {
    const pool = getDbPool();
    const [platformRows] = await pool.query("SELECT * FROM gaming_platforms");
    if (Array.isArray(platformRows) && platformRows.length > 0) {
      statePlatforms = platformRows.map((row) => {
        if (row.platform_json) {
          try {
            return JSON.parse(row.platform_json);
          } catch (e) {
          }
        }
        return {
      ...
      for (const p of initialPlatforms) {
        await pool.query(
          ...
          [
            ...
            p.metaDescription || "",
            p.metaKeywords || "",
            p.averageUserRating || 4.5,
            p.totalReviewsCount || 0,
            JSON.stringify(p)
          ]
        );
      }
    ...
    const [configRows] = await pool.query("SELECT config_json FROM global_config WHERE id = 1 LIMIT 1");
    ...
    const [subPartnerRows] = await pool.query("SELECT * FROM sub_partner_applications ORDER BY created_at DESC");
    if (Array.isArray(subPartnerRows) && subPartnerRows.length > 0) {
      stateSubPartners = subPartnerRows.map((r) => ({
        id: r.id,
        fullName: r.fullName,
        email: r.email,
        whatsapp: r.whatsapp,
    ...
    if (Array.isArray(pageRows) && pageRows.length > 0) {
      stateCustomPages = pageRows.map((r) => {
        if (r.page_json) {
          try {
            return JSON.parse(r.page_json);
          } catch (e) {
          }
        }
        return {
    ...
    const [statsRows] = await pool.query("SELECT * FROM analytics_stats WHERE id = 1 LIMIT 1");
    if (Array.isArray(statsRows) && statsRows.length > 0) {
      ...
      if (s.platformStats) {
        try {
          stateStats.platformStats = JSON.parse(s.platformStats);
        } catch (e) {
        }
      }
    }
...
async function savePlatformsToDb(platforms) {
  try {
    const pool = getDbPool();
...
async function saveCustomPagesToDb(pages) {
  try {
    const pool = getDbPool();
...
async function insertSubPartnerToDb(app2) {
  try {
    const pool = getDbPool();
    await pool.query(
      `INSERT INTO sub_partner_applications 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [app2.id, app2.fullName, app2.email, app2.whatsapp, app2.platformId, app2.platformName, app2.trafficSource, app2.estimatedMonthlyPlayers, app2.status, app2.appliedAt]
    );
  } catch (err) {
    logger.error("[MySQL] Error inserting sub-partner to MySQL:", err.message);
  }
}
async function updateSubPartnerStatusInDb(id, status) {
  try {
    const pool = getDbPool();
...
async function deleteSubPartnerFromDb(id) {
  try {
    const pool = getDbPool();
...
async function insertTrackLogToDb(log) {
  try {
    const pool = getDbPool();
    await pool.query(
...
function getGeoFromRequest(req) {
  ...
  if (countryHeader && countryHeader !== "XX") {
    return {
      ...
      city: "Detected City",
      ip,
      flag: countryHeader === "IN" ? "\u{1F1EE}\u{1F1F3}" : countryHeader === "US" ? "\u{1F1FA}\u{1F1F8}" : countryHeader === "BR" ? "\u{1F1E7}\u{1F1F7}" : "\u{1F310}"
    };
  }
  return {
...
var adminLoginRateLimiter = (req, res, next) => {
  ...
  if (record.lockUntil > now) {
    const remainingSeconds = Math.ceil((record.lockUntil - now) / 1e3);
    res.setHeader("Retry-After", remainingSeconds);
    res.setHeader("X-RateLimit-Limit", maxAttempts);
    res.setHeader("X-RateLimit-Remaining", 0);
    return res.status(429).json({
      success: false,
      message: `\u{1F512} BRUTE-FORCE LOCKOUT: Too many failed admin login attempts from IP ${clientIp}. Access blocked for ${remainingSeconds} seconds.`
    });
  }
  record.attempts = record.attempts.filter((timestamp) => now - timestamp < windowMs);
  if (record.attempts.length >= maxAttempts) {
    record.lockUntil = now + 15 * 60 * 1e3;
    res.setHeader("Retry-After", 900);
    return res.status(429).json({
      success: false,
      message: `\u{1F512} RATE LIMIT EXCEEDED: 5 failed attempts reached from IP ${clientIp}. Blocked for 15 minutes.`
    });
  }
  res.setHeader("X-RateLimit-Limit", maxAttempts);
...
var handleAdminLogin = (req, res) => {
  ...
  if (password === ADMIN_PASSCODE) {
    loginAttemptTracker[clientIp] = { attempts: [], lockUntil: 0 };
    const token = import_jsonwebtoken.default.sign({ role: "admin", authAt: Date.now() }, JWT_SECRET, { expiresIn: "8h" });
    return res.json({ success: true, token });
  } else {
    record.attempts.push(Date.now());
    if (record.attempts.length >= 5) {
      record.lockUntil = Date.now() + 15 * 60 * 1e3;
    }
    ...
    setTimeout(() => {
      ...
      return res.status(401).json({
        success: false,
        message: record.attempts.length >= 5 ? "\u{1F512} Account locked for 15 minutes due to 5 failed password attempts." : `Invalid passcode! Security Warning: ${remaining} attempt(s) remaining before IP lockout.`
      });
    }, 500);
  }
};
...
app.get("/api/postback/:platform", async (req, res) => {
  ...
  if (!platform || !secret || secret !== platform.postbackKey) {
...
app.post("/api/admin/custom-pages", verifyJwtToken, import_express.default.json(), async (req, res) => {
...
app.get("/go/:slug", (req, res) => {
  ...
  if (clickId || sub1 || sub2) {
    ...
    if (clickId) urlObj.searchParams.set("click_id", clickId);
    ...
    <body class="bg-slate-950 text-white font-sans min-h-screen flex items-center justify-center p-4">
      <div id="cardBox" class="max-w-md w-full bg-slate-900 border-2 border-emerald-500/70 rounded-3xl p-6 shadow-2xl text-center space-y-5 relative overflow-hidden transition-all duration-500">
        ...
        <div id="timerBadge" class="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-emerald-950 border border-emerald-500/50 text-emerald-300 text-xs font-black uppercase tracking-wider transition-all">
          <span>\u{1F7E2} 10-MINUTE REGISTRATION TIMER ACTIVATED</span>
        </div>
        ...
        setInterval(() => {
          if (totalSeconds > 0) {
            ...
            // Phase 3: < 3m (0s - 180s) -> Crimson Red Urgent Rapid Pulse / Bounce
              ...
              timerLabel.innerText = "\u{1F6A8} CRITICAL WARNING - EXPIRING IN MINUTES!";
              ...
              timerBadge.innerText = "\u{1F6A8} CRITICAL WARNING";
            }
          }
        }, 1000);
    ...
    const response = await generateWithRetry(ai, {
      ...
      config: {
        ...
        responseSchema: {
          type: import_genai.Type.OBJECT,
          properties: {
            title: { type: import_genai.Type.STRING, description: "A catchy, SEO-friendly H1 title" },
            metaTitle: { type: import_genai.Type.STRING, description: "SEO Meta Title (max 60 chars)" },
            metaDescription: { type: import_genai.Type.STRING, description: "SEO Meta Description (max 160 chars)" },
            content: { type: import_genai.Type.STRING, description: "The full article content in Markdown format" },
            tags: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING }, description: "5-7 relevant SEO tags/keywords" }
          },
...
async function startServer() {
  ...
  if (!isProduction) {
    ...
    const distPath = candidates.find((c) => fs.existsSync(import_path.default.join(c, "index.html")) && fs.existsSync(import_path.default.join(c, "assets"))) || candidates.find((c) => fs.existsSync(import_path.default.join(c, "index.html"))) || import_path.default.join(process.cwd(), "dist");
    ...
    app.get("*", (req, res) => {
      ...
      res.setHeader("Pragma", "no-cache");
      res.setHeader("Expires", "0");
      const htmlFile = import_path.default.join(distPath, "index.html");
  ...
  const autoblogInterval = setInterval(async () => {
    ...
    Return ONLY valid JSON in this exact format:
    {
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              metaTitle: { type: import_genai.Type.STRING },
      ...
      if (response.text) {
        ...
        const newArticle = {
          id: "art_auto_" + Math.floor(Math.random() * 1e6),
          slug: data2.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)+/g, ""),
          title: data2.title,
          content: data2.content,
          category,
          metaTitle: data2.metaTitle,
          metaDescription: data2.metaDescription,
          publishedAt: (/* @__PURE__ */ new Date()).toISOString(),
        ...
        logger.info(`[Auto-Blogger] Successfully created draft: ${data2.title}`);
      }
    } catch (err) {
      logger.error("[Auto-Blogger] Error generating article:", err);
    }
  }, (stateConfig.autoBlogSettings?.intervalHours || 24) * 60 * 60 * 1e3);
  if (process.env.SENTRY_DSN) {
